...
class Oscilloscope:
    def __init__(self, name='Keysight MSO-x 4054', addr='192.168.2.10:5025'):
        self.addr = addr
        self.ss = None
        self.name = name
        self.fileSuffix = '.1'
    ...
    def send(self, cmd):
        ...
        cmd2 = bytes(cmd+self.cmdSuffix,encoding='UTF-8')
        self.ss.send(cmd2)                           #read back device ID
    ...
    def takeData2(self, channels=[1], filename='temp1.dat'):
        ...
        for iChan in channels: 
            ...
            ### meta data
            self.send(":WAVeform:PREamble?;")
            a0 = ss.recv(256) ### the data is longer than 128
            a = a0.decode() 
            print(a)
            pre = a.split(',')
...
def test1():
    oc1 = Oscilloscope('Agilent MSO9104A',addr='192.168.2.10:5025')
#     oc1.test(0)
    oc1.takeData2([1,2])
